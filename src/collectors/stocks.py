...
def collect_stocks(trading_date: date | None = None) -> dict[str, Any]:
    trading_date = trading_date or today_et()
    prior_day = prior_trading_day(trading_date)
    cfg = load_symbols()
    batch = fetch_many(cfg["stocks"], trading_date=trading_date)
    quotes = batch["quotes"]
    checklist = {
        sym: _quote_ok(quotes, sym, trading_date=trading_date, prior_day=prior_day)
